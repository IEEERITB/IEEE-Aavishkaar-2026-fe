...
export interface RegistrationPayload {
  ...
  leadPhone: string
  leadUSN: string
  teamMembers: Array<{ name: string; usn?: string }>
}
...
export async function submitRegistration(
  ...
  if (!base) {
    await delay(400)
    const ref = `TF-${Math.random().toString(36).slice(2, 8).toUpperCase()}`
    return { ok: true, referenceId: ref }
  }
  const res = await fetch(`${base}/api/registrations/${encodeURIComponent(eventId)}`, {
    method: 'POST',
